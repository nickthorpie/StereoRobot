...
def filter_colors(image):
	...
	lower_yellow = np.array([14,28,80])
	upper_yellow = np.array([34,222,245])
...
if __name__ == '__main__':
    ...
    cv2.namedWindow("laneline")
    cv2.namedWindow("lineimage")
    while True:
        ret,frame = cam.read()
        image_in = frame
        image = filter_colors(image_in)
        ...
        cv2.imshow('lineimage',masked_edges)
        cv2.imshow('laneline',annotated_image)
